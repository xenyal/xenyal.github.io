...
Network.prototype.checkLines = function() {
    ...
    for (var i = this.aDots.length - 1; i >= 0; i--) {
        ...
        for (var j = this.aDots.length - 1; j >= 0; j--) {

            var iDist = this.getDist(this.aDots[i], this.aDots[j]);

            if (iDist < this.iMaxDist && iDist > this.iMinDist && (this.aDots[i].r + this.aDots[j].r) > 5.5 && j > i) {

                this.addLine(this.aDots[i], this.aDots[j], iDist);
...
Network.prototype.addLine = function(dot1, dot2, iDist) {

    var sName = dot1.x + dot1.y + dot2.x + dot2.y;

    this.aLines[sName] = new Line(dot1, dot2, iDist, this.iMaxDist);
...
Network.prototype.drawOverlay = function() {
    ...
    for (var i = 0; i < breakpoint; i++) {
        ...
        var h = (360 / breakpoint) * i;
        var color = 'hsla( ' + h + ', 100%, 50%, 1 )';
        this.grd.addColorStop((1 / breakpoint) * i, color);
...
Network.prototype.draw = function() {
    ...
    if (this.drawLine) {
        ...
        for (var key in this.aLines) {

            this.aLines[key].draw(this.ctx);
...
var Dot = function(mouse) {
    ...
    this.x = mouse.x;
    this.y = mouse.y;
    this.vx = rand(mouse.vx - 2, mouse.vx + 2) * 1.5;
    this.vy = rand(mouse.vy - 2, mouse.vy + 2) * 1.5;
    this.r = rand(0, 4);
    this.life = rand(1, 5) * (this.r * .3);
    this.alive = true;
    this.friction = rand(0.05, 0.1);
